...
def get_probs(pulls):
    pulls_pities = np.array(
        pulls[(pulls['rarity']==5)&(pulls['guaranteed']=='f')&(pulls['type']=='character')]['pity'])
...
def fit_mixture(probs, gaussian_mean, gaussian_var, cutoff):
    ...
    bin_discrete_sum = sum([((1-p)**i)*p for i in range(0,cutoff)])
    bin_scale = 1/bin_discrete_sum
    
    print('gauss sum:',norm_discrete_sum)
    print('bin sum:', bin_discrete_sum)
    
    for i in range(89):
        if i<cutoff:
            output.append(a*bin_scale*((1-p)**i)*p)
        else:
